...
int main ()
{
	...
	int nind=0; // real number of individuals
	int nbm=0; // real number of markers
	int incomp=0, fam=0, multi, aff, sain, nreal, nmulti;
	int cov; // real number of covariates
...
/* -- File reading -- */
	file_in = fopen (ficgeno, "r"); // Open imput file
	printf("Arquivo aberto\n");
	i=0;
	fscanf (file_in, "%d", &geno[i][0]); 
	while (feof (file_in) == 0)
	{	
		for (j=1; j<(2*nbm + 6 + cov); j++) 
		{
			fscanf (file_in, "%d", &geno[i][j]);
		}
		i++;
		fscanf (file_in, "%d", &geno[i][0]); 
	}
 ...
 /* lecture de la structure et des génotypes des famille pour repérage des familles de type sib_tdt */
	...
	{ /*1*/
		...
		{ /*2 vérification du génotypage des parents et enfants*/
			...
			{
				for (j=1; j<2*nbm; j=j+2)
				{
					geno_pere=geno_pere+geno[i+k][6+cov+j];
				}
			}
			if ((geno[i+k][3]==0) && (geno [i+k][4]==2))
			{
				for (j=1; j<2*nbm; j=j+2)
				{
					geno_mere=geno_mere+geno[i+k][6+cov+j];
				}
			}
		 /* comptage des enfants atteints génotypés*/
			if ((geno[i+k][3]!=0) && (geno [i+k][5]==2)) 
			{
				for (j=1; j<2*nbm; j=j+2)
				{
					geno_aff=geno_aff + geno[i+k][6+cov+j];
				}
				if (geno_aff != 0)
				{
					aff=aff + 1;
		 ...
		 /* comptage des enfants sains génotypés*/
			if ((geno[i+k][3]!=0) && (geno [i+k][5]==1)) 
			{
				for (j=1; j<2*nbm; j=j+2)
				{
					geno_sain=geno_sain + geno[i+k][6+cov+j];
				}
				if (geno_sain != 0)
				{
					sain=sain + 1;
		...
		{ /*2*/
			...
			{/*3 si trio avec 2 parents génotypés */
				...
				{ /*4 lecture de la structure et des génotypes de la famille */
					if ((geno[i+k][3]==0) && (geno [i+k][4]==1))
					{
						geno_pere=geno[i+k][6+cov+j]+geno[i+k][7+cov+j];
					}
					if ((geno[i+k][3]==0) && (geno [i+k][4]==2))
					{
						geno_mere=geno[i+k][6+cov+j]+geno[i+k][7+cov+j];
					}
				} /*4*/
